...
import { useLazyGetOneBlogQuery } from "@/api/blog";
...
const BlogCard = ({ blogItem, cardClick }: Props) => {
  ...
  //state
  const { isOpeningOneBlog, setIsOpeningOneBlog } = useContext(
    IsOpeningOneBlogContext
  );
  ...
  //queries
  const [triggerGetOneBlog, { data: blogData }] =
  ...
  useEffect(() => {
    ...
    const handleCloseBlog = () => {
      ...
      card_container?.classList.remove("tw-h-[100dvh]");
    };
    const handleOpenBlog = () => {
      if (card && card_container && !isOpeningOneBlog) {
        ...
        card_container.classList.add("tw-h-[100dvh]");
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="tw-overflow-scroll tw-absolute tw-top-0 tw-left-0 tw-right-0 tw-w-full tw-h-[100dvh] tw-z-40"
        id="blog-container"
      >
        <div className="tw-w-full tw-h-[calc(100dvh_-_80px)] tw-bg-opacity-0"></div>
